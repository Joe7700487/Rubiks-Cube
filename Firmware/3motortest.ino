...
#define DELAY 50
#define POSTDELAY 250

#define STEPPERACC 8000
#define STEPPERSPEED 6000

#define HANDOFF 0 // off position of hands
#define NORMAL 90 // single layer position
#define WIDE 130 // double and triple layer position
...
AccelStepper stepper1(1, 26, 25); // R
AccelStepper stepper2(1, 32, 33); // U
AccelStepper stepper3(1, 14, 27); // B
...
void turnFace(String move) {
  ...
  if (layers == 1) {
    ...
    motor[side]->moveTo(motor[side]->currentPosition() + amount);
    while(motor[side]->distanceToGo() != 0) {
      motor[side]->run();
    }
